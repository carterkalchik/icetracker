...
export function commitAndPush(files: string[], message: string): void {
  ...
  run('git', ['commit', '-m', message, '--author', 'BladeTracker Crawler <crawler@bladetracker.app>'])
...
export function createPR(opts: {
  ...
  run('git', ['commit', '-m', opts.title, '--author', 'BladeTracker Crawler <crawler@bladetracker.app>'])
  run('git', ['push', '-u', 'origin', opts.branch])

  // Create PR
  const ghArgs = ['pr', 'create', '--title', opts.title, '--body', opts.body]
  if (opts.labels) {
    for (const label of opts.labels) {
      ghArgs.push('--label', label)
    }
  }
  const prUrl = run('gh', ghArgs)
